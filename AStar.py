...
class Path:
    def __init__(self):
        ...
        self.pathQueue = queue.PriorityQueue()
        self.end = snake.food
    ...
    def findPath(self):
        ...
        while True:
            self.currentPath = self.pathQueue.get()[1]
            ...
            if self.pathQueue.empty():
                self.map = snake.getArray()
                self.checkNeighbours("")
                return self.notations[self.pathQueue.get()[1]]
        ...
        for direction, _ in self.notations.items():
            ...
            if 0 <= currentPoint[0] < 30 and 0 <= currentPoint[1] < 30:
                if self.map[currentPoint] == 0:
                    # score = math.sqrt((self.end[0]-currentPoint[0])**2 + (self.end[1]-currentPoint[1])**2) + math.sqrt((self.start[0]-currentPoint[0])**2 + (self.start[1]-currentPoint[1])**2)
                    score = math.sqrt((self.end[0]-currentPoint[0])**2) + math.sqrt((self.end[1]-currentPoint[1])**2) + math.sqrt((self.start[0]-currentPoint[0])**2) + math.sqrt((self.start[1]-currentPoint[1])**2)
                    self.pathQueue.put((score, tempPath))
